...
ma['50day'] = df.Close.rolling(50).mean().shift()
ma['200day'] = df.Close.rolling(200).mean().shift()
...
length = len(df)
forecast_col = 'Close'
forecast_out = int(math.ceil(0.1*length))
...
for i in prediction:
    nextday = datetime.datetime.fromtimestamp(nextraw)
    nextraw += day
...
plt.plot(ma['50day'], label='50 day')
plt.plot(ma['200day'], label='200 day')
